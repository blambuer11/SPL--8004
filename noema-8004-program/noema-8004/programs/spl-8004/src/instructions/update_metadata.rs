...
pub struct UpdateMetadata<'info> {
    #[account(mut, has_one = owner @ SPL8004Error::Unauthorized, constraint = identity.is_active @ SPL8004Error::AgentNotActive)]
    pub identity: Account<'info, IdentityRegistry>,
    pub owner: Signer<'info>,
}

pub fn handler(ctx: Context<UpdateMetadata>, new_metadata_uri: String) -> Result<()> {
    require!(new_metadata_uri.len() <= MAX_METADATA_URI_LEN, SPL8004Error::MetadataUriTooLong);
