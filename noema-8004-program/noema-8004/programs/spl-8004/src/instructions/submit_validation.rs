...
pub struct SubmitValidation<'info> {
    #[account(init, payer = validator, space = 8 + ValidationRegistry::LEN, seeds = [VALIDATION_SEED, agent.agent_id.as_bytes(), task_hash.as_ref()], bump)]
...
pub fn handler(
    ...
    require!(evidence_uri.len() <= MAX_EVIDENCE_URI_LEN, SPL8004Error::EvidenceUriTooLong);
    ...
    validation.bump = *ctx.bumps.get("validation").expect("missing bump: validation");

    config.total_validations = config.total_validations.checked_add(1).ok_or(SPL8004Error::ArithmeticOverflow)?;
