...
pub struct UpdateReputation<'info> {
    #[account(mut, seeds = [REPUTATION_SEED, agent.agent_id.as_bytes()], bump = reputation.bump, constraint = reputation.agent == agent.key())]
    ...
    #[account(constraint = validation.agent == agent.key())]
    pub validation: Account<'info, ValidationRegistry>,

    #[account(mut, seeds = [REWARD_POOL_SEED, agent.agent_id.as_bytes()], bump = reward_pool.bump)]
...
pub fn handler(ctx: Context<UpdateReputation>) -> Result<()> {
    ...
    reputation.total_tasks = reputation.total_tasks.checked_add(1).ok_or(SPL8004Error::ArithmeticOverflow)?;

    let score_change = reputation.calculate_score_change(validation.approved);

    if validation.approved {
        reputation.successful_tasks = reputation.successful_tasks.checked_add(1).ok_or(SPL8004Error::ArithmeticOverflow)?;

        let new_score = (reputation.score as i64).checked_add(score_change).ok_or(SPL8004Error::ArithmeticOverflow)?;
        ...
        reward_pool.claimable_amount = reward_pool.claimable_amount.checked_add(reward).ok_or(SPL8004Error::ArithmeticOverflow)?;

        msg!("Task approved! Score increased by {}", score_change);
        msg!("Reward added: {} lamports", reward);
    } else {
        reputation.failed_tasks = reputation.failed_tasks.checked_add(1).ok_or(SPL8004Error::ArithmeticOverflow)?;
...
fn calculate_reward(current_score: u64, score_increase: u64) -> u64 {
    let base_reward = 100_000; // 0.0001 SOL in lamports
