...
import { StakingConfig, ValidatorAccount } from '@/lib/staking-client';
...
export default function Staking() {
  ...
  const [noemaInstantUnstakingBusy, setNoemaInstantUnstakingBusy] = useState(false);
  ...
  const handleStake = async () => {
    if (!stakingClient || !publicKey) {
      toast.error('Connect your wallet to stake SOL');
      return;
    }
    ...
    setStaking(true);
    const loadingId = toast.loading('🔄 Staking SOL tokens...', { duration: Infinity });
    try {
      const lamports = Math.floor(parsed * LAMPORTS_PER_SOL);
      const sig = await stakingClient.stake(lamports);
      toast.dismiss(loadingId);
      toast.success('✅ SOL staked successfully!', {
        description: (
  ...
  const handleUnstake = async (instant = false) => {
    if (!stakingClient || !publicKey) {
      toast.error('Connect your wallet to unstake SOL');
      return;
    }
    ...
    setUnStaking(true);
    const loadingId = toast.loading(`🔄 ${instant ? 'Instant unstaking' : 'Unstaking'} SOL...`, { duration: Infinity });
    try {
      const lamports = Math.floor(parsed * LAMPORTS_PER_SOL);
      const sig = instant
      ...
      toast.dismiss(loadingId);
      toast.success(`${instant ? '⚡ Instant unstake' : '✅ Unstake'} successful!`, {
        description: (
          <a href={getExplorerTxUrl(sig)} target="_blank" rel="noopener noreferrer" className="underline">
            View transaction →
          </a>
      ...
      toast.error(error instanceof Error ? error.message : '❌ Failed to unstake SOL');
    } finally {
      setUnStaking(false);
    }
  };
  ...
  const handleClaim = async () => {
    ...
    setClaiming(true);
    const loadingId = toast.loading('🔄 Claiming SOL rewards...', { duration: Infinity });
    try {
  ...
  const handleNoemaStake = async () => {
    if (!noemaClient || !publicKey) { toast.error('Connect wallet to stake NOEMA'); return; }
    const parsed = parseFloat(noemaStakeAmount); if (!parsed || parsed <= 0) { toast.error('Enter valid NOEMA amount'); return; }
    setNoemaStakingBusy(true); const loadingId = toast.loading('🔄 Staking NOEMA tokens...', { duration: Infinity });
    try {
      const raw = BigInt(Math.floor(parsed * 1e9)); const sig = await noemaClient.stake(raw);
      toast.dismiss(loadingId); 
      toast.success('✅ NOEMA staked successfully!', { 
        description: <a href={getExplorerTxUrl(sig)} target="_blank" rel="noopener noreferrer" className="underline">View transaction →</a>,
        duration: 5000
      });
      setNoemaStakeAmount(''); await refreshData();
    } catch (e) { 
      toast.dismiss(loadingId); 
      toast.error(e instanceof Error ? e.message : '❌ NOEMA staking failed'); 
    } finally { setNoemaStakingBusy(false); }
  };

  const handleNoemaUnstake = async () => {
    if (!noemaClient || !publicKey) { toast.error('Connect wallet to unstake NOEMA'); return; }
    const parsed = parseFloat(noemaUnstakeAmount); if (!parsed || parsed <= 0) { toast.error('Enter valid NOEMA amount'); return; }
    if (noemaValidator && parsed > Number(noemaValidator.stakedAmount)/1e9) { toast.error('Amount exceeds staked balance'); return; }
    setNoemaUnstakingBusy(true); const loadingId = toast.loading('🔄 Processing NOEMA unstake...', { duration: Infinity });
    try {
      const raw = BigInt(Math.floor(parsed * 1e9)); const sig = await noemaClient.unstake(raw);
      toast.dismiss(loadingId); 
      toast.success('✅ NOEMA unstaked successfully!', { 
        description: <a href={getExplorerTxUrl(sig)} target="_blank" rel="noopener noreferrer" className="underline">View transaction →</a>,
        duration: 5000
      });
      ...
      toast.dismiss(loadingId); 
      toast.error(e instanceof Error ? e.message : '❌ NOEMA unstake failed'); 
    } finally { setNoemaUnstakingBusy(false); }
  };

  const handleNoemaInstantUnstake = async () => {
    if (!noemaClient || !publicKey) { toast.error('Connect wallet for NOEMA instant unstake'); return; }
    if (!noemaConfig) { toast.error('NOEMA config not found'); return; }
    const parsed = parseFloat(noemaUnstakeAmount); if (!parsed || parsed <= 0) { toast.error('Enter valid NOEMA amount'); return; }
    if (noemaValidator && parsed > Number(noemaValidator.stakedAmount)/1e9) { toast.error('Amount exceeds staked balance'); return; }
    setNoemaInstantUnstakingBusy(true); const loadingId = toast.loading('🔄 NOEMA instant unstake processing...', { duration: Infinity });
    try {
      ...
      toast.dismiss(loadingId); 
      toast.success('⚡ Instant unstake successful!', { 
        description: <a href={getExplorerTxUrl(sig)} target="_blank" rel="noopener noreferrer" className="underline">View transaction →</a>,
        duration: 5000 
      });
      setNoemaUnstakeAmount(''); await refreshData();
      ...
      toast.dismiss(loadingId); 
      toast.error(e instanceof Error ? e.message : '❌ NOEMA instant unstake failed'); 
    } finally { setNoemaInstantUnstakingBusy(false); }
  };

  const handleNoemaClaim = async () => {
    if (!noemaClient || !publicKey) { toast.error('Connect wallet to claim NOEMA rewards'); return; }
    if (!noemaPendingRewards || noemaPendingRewards <= 0n) { toast.info('No NOEMA rewards available yet'); return; }
    setNoemaClaimingBusy(true); const loadingId = toast.loading('🔄 Claiming NOEMA rewards...', { duration: Infinity });
    try {
      const sig = await noemaClient.claimRewards();
      toast.dismiss(loadingId); 
      toast.success('🎉 NOEMA rewards claimed!', { 
        description: <a href={getExplorerTxUrl(sig)} target="_blank" rel="noopener noreferrer" className="underline">View transaction →</a>,
        duration: 5000
      });
      await refreshData();
      ...
      toast.dismiss(loadingId); 
      toast.error(e instanceof Error ? e.message : '❌ NOEMA claim failed'); 
    } finally { setNoemaClaimingBusy(false); }
  };
      ...
      <div className="space-y-6">
        ...
        {!noemaConnected ? (
          ...
          <>
            ...
            <Card className="bg-white/5 border-white/10">
              <CardHeader>
                <CardTitle className="text-white text-base">Stake NOEMA</CardTitle>
                <CardDescription>Lock NOEMA tokens to earn rewards and build reputation</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid gap-3 md:grid-cols-[1fr_auto]">
                  <Input
                    ...
                    placeholder="Amount (NOEMA)"
                    className="bg-white/10 border-white/20 text-white"
                  />
                  <Button
                    onClick={handleNoemaStake}
                    disabled={noemaStakingBusy || !noemaStakeAmount}
                    className="bg-purple-600 hover:bg-purple-700"
                  >
                    <ArrowUpCircle className="w-4 h-4 mr-2" />
                ...
                <p className="text-xs text-slate-500">
                  {noemaConfig && `Min stake: ${(Number(noemaConfig.validatorMinStake) / 1e9).toFixed(2)} NOEMA • APY: ${noemaConfig.baseApyBps / 100}%`}
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-white/5 border-white/10">
              <CardHeader>
                <CardTitle className="text-white text-base">Unstake NOEMA</CardTitle>
                <CardDescription>Standard unstake respects cooldown • instant unstake charges fee</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid gap-3 md:grid-cols-[1fr_auto_auto]">
                  <Input
                  ...
                  <Button
                    variant="outline"
                    onClick={handleNoemaUnstake}
                    disabled={noemaUnstakingBusy || !noemaUnstakeAmount}
                    className="border-white/20 text-slate-200 hover:text-white"
                  >
                    <ArrowDownCircle className="w-4 h-4 mr-2" />
                  ...
                  <Button
                    onClick={handleNoemaInstantUnstake}
                    disabled={noemaInstantUnstakingBusy || !noemaUnstakeAmount}
                    className="bg-purple-600 hover:bg-purple-700"
                  >
                    <ArrowDownCircle className="w-4 h-4 mr-2" />
                    {noemaInstantUnstakingBusy ? 'Processing…' : 'Instant Unstake NOEMA'}
                  </Button>
                </div>
                <p className="text-xs text-slate-500">
                  {noemaConfig && `Instant fee: ${noemaConfig.instantUnstakeFeeBps / 100}% • Staked: ${noemaValidator ? (Number(noemaValidator.stakedAmount) / 1e9).toFixed(4) : '0.0000'} NOEMA`}
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-white/5 border-white/10">
              <CardHeader>
                <CardTitle className="text-white text-base">Claim NOEMA Rewards</CardTitle>
                <CardDescription>Collect your earned NOEMA tokens</CardDescription>
              </CardHeader>
              <CardContent className="flex items-center justify-between">
                <div>
                  <div className="text-2xl font-semibold text-purple-300">
                    {(Number(noemaPendingRewards) / 1e9).toFixed(4)} NOEMA
                  </div>
                  <div className="text-xs text-slate-500 mt-1">Pending rewards</div>
                </div>
                <Button
                  onClick={handleNoemaClaim}
                  disabled={noemaClaimingBusy || !noemaPendingRewards || noemaPendingRewards <= 0n}
                  className="bg-purple-600 hover:bg-purple-700"
                >
                  {noemaClaimingBusy ? 'Claiming…' : 'Claim Rewards'}
                </Button>
              </CardContent>
            </Card>
